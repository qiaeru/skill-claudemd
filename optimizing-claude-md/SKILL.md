...
1. **Read every memory file in scope.** Load the target CLAUDE.md and any others in the hierarchy that apply (parent directories, nested CLAUDE.md, `.claude/rules/`). Record the current line count so you can report the reduction.
...
7. **Relocate misplaced content.** Move multi-step procedures and rules that only matter for one part of the codebase out of CLAUDE.md into a skill or a path-scoped rule, so they load only when relevant.
...
This is the core move and the one most often done wrong. There are four mechanisms, and they do not all save tokens.
...
- **Path-scoped rule** (`.claude/rules/*.md` with a `paths:` frontmatter field). Loads only when Claude touches matching files. Use for conventions tied to one file type or directory.
...
- **HTML comments are free.** Block-level `<!-- ... -->` comments are stripped before the file enters context, so use them for maintainer notes without spending tokens.
...
- Multi-step or path-specific content moved to a skill or a path-scoped rule.
